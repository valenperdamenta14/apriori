...
exports.getById = (req, res) => {
  ...
  db.query('SELECT * FROM data_transaksi WHERE id_transaksi = ?', [id], (err, result) => {
    if (err) return res.status(500).json(err);
    res.json(result[0]);
  });
};
...
exports.update = (req, res) => {
  ...
  db.query('UPDATE data_transaksi SET ? WHERE id_transaksi=?', [data, id], (err) => {
    if (err) return res.status(500).json(err);
...
exports.delete = (req, res) => {
  ...
  db.query('DELETE FROM data_transaksi WHERE id_transaksi=?', [id], (err) => {
    if (err) return res.status(500).json(err);
